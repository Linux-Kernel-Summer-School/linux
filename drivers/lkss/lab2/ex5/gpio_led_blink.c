...
#include <linux/jiffies.h>		/* For jiffies and time conversions */
...
static struct timer_list blink_timer;	/* Timer for blinking functionality */
static int blink_active = 0;		/* Flag to indicate if blinking is active */
static int led_state = 0;		/* Current LED state (on/off) */

/**
 * Timer callback: toggles LED and re-arms if blinking is active
 */
static void blink_timer_callback(struct timer_list *t)
{
	led_state = !led_state;/* Toggle LED state */
	gpiod_set_value(led_gpio, led_state);/* Set the GPIO to new state */

	if (blink_active)/* If blinking is active */
		mod_timer(&blink_timer, jiffies + msecs_to_jiffies(500));/* Restart timer for 500ms later */
}

/**
 * Write function: control LED (on/off) or blinking
 */
static ssize_t gpio_led_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	char kbuf[8];/* Kernel buffer to store led state and null terminator */

	if (count >= sizeof(kbuf))/* Prevent buffer overflow */
		return -EINVAL;

	if (copy_from_user(kbuf, buf, count))/* Copy data from user space */
		return -EFAULT;

	kbuf[count] = '\0';/* Null-terminate buffer */

	/* Turn LED on */
	if (kbuf[0] == '1') {
		timer_delete_sync(&blink_timer);/* Stop blinking */
		blink_active = 0;
		gpiod_set_value(led_gpio, 1);/* Turn on LED */
		led_state = 1;
	/* Turn LED off */
	} else if (kbuf[0] == '0') {
		timer_delete_sync(&blink_timer);/* Stop blinking */
		...
		led_state = 0;
	/* Start blinking mode */
	} else if (strncmp(kbuf, "b", 1) == 0 || strncmp(kbuf, "blink", 5) == 0) {
		blink_active = 1;
		mod_timer(&blink_timer, jiffies + msecs_to_jiffies(500));/* Start blinking in 500ms */
	}
...
static void gpio_led_remove(struct platform_device *pdev)
{
	...
	timer_delete_sync(&blink_timer);	/* Stop and clean up timer */
}
...
static struct platform_driver gpio_led_driver = {
	...
	.driver = {
		.name = "gpio-led-blink",/* Name of the driver */
...
MODULE_DESCRIPTION("Lab2 Ex5: Character device LED driver using gpiod and a kernel timer to implement blinking.");
