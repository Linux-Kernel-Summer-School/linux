...
#define GPIO_LED_ON	_IO(GPIO_LED_MAGIC, 1)	/* IOCTL command to turn LED on */
#define GPIO_LED_OFF	_IO(GPIO_LED_MAGIC, 2)	/* IOCTL command to turn LED off */
 ...
 *
 * unlocked_ioctl has the format (struct file *file,unsigned int cmd,unsigned long arg)
