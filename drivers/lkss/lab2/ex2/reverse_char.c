...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset) {
	...
	reverse_string(message);/* Reverse the message in place */
...
static int __init reverse_init(void) {
	/* Register character device and get dynamically assigned major number */
	major = register_chrdev(0, DEVICE_NAME, &fops);
	if (major < 0)
		return major;

	/* Create a device class in sysfs (/sys/class/rev_class) */
	rev_class = class_create(CLASS_NAME);
	if (IS_ERR(rev_class)) {
		unregister_chrdev(major, DEVICE_NAME);
		return PTR_ERR(rev_class);
	}

	/* Create the device file (/dev/reverse_char) */
	rev_device = device_create(rev_class, NULL, MKDEV(major, 0), NULL, DEVICE_NAME);
	if (IS_ERR(rev_device)) {
		class_destroy(rev_class);
		unregister_chrdev(major, DEVICE_NAME);
		return PTR_ERR(rev_device);
	}
...
static void __exit reverse_exit(void) {
	device_destroy(rev_class, MKDEV(major, 0));/* Remove /dev entry */
	class_unregister(rev_class);/* Unregister class */
	class_destroy(rev_class);/* Destroy class */
